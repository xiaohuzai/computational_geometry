...
class arrow():
    ...
    def plt_arrow(self, ax):
        ...
        #设定两个变换，一个使绘制出的箭头大小不依赖于坐标系而是屏幕，一个将箭头移至坐标系中的指定坐标
        trans = (
            ax.figure.dpi_scale_trans +
            transforms.ScaledTranslation(self.x[1], self.y[1], ax.transData))
        theta = self.calc_theta(ax, self.x, self.y)
        #将坐标和旋转矩阵相乘，加上的偏移量[[0.01],[0]]目的是解决绘制的箭头处于线条终点时顶端两侧由于线宽导致的像素露出
        coords = np.array([[cos(theta), -sin(theta)], [sin(theta),
                                                       cos(theta)]
                           ]) @ (np.array([X, Y]) + np.array([[0.01], [0]]))
        self.handle = ax.fill(coords[0, :],
                              coords[1, :],
                              color=self.color,
                              **self.kw,
                              transform=trans)[0]
...
class arrowline():
    ...
    def __init__(self,
                 ...
                 **kw):
        self.arrows = []
        #对当前ax进行初始化设置，增加一些属性，对axes实例的方法进行一些增添修改
        arrowline.set_ax(ax)
        ...
        if style == 'to':
            l -= 1
            self.arrows.append(
                arrow(ax, [x[l - 1], x[l]], [y[l - 1], y[l]], arrow_size,
                      **kw))
        elif style == 'to_back':
            l -= 1
            self.arrows.append(
                arrow(ax, [x[l - 1], x[l]], [y[l - 1], y[l]], arrow_size,
                      **kw))
            self.arrows.append(
                arrow(ax, [x[1], x[0]], [y[1], y[0]], arrow_size, **kw))
        elif style == 'middle':
            if l % 2 == 0:
                l = int(l / 2)
                self.arrows.append(
                    arrow(ax, [x[l - 1], (x[l - 1] + x[l]) / 2],
                          [y[l - 1], (y[l - 1] + y[l]) / 2], arrow_size, **kw))
            else:
                l = int(l / 2)
                self.arrows.append(
                    arrow(ax, [x[l - 1], x[l]], [y[l - 1], y[l]], arrow_size,
            ...
            while i < l:
                self.arrows.append(
                    arrow(ax, [x[i - 1], x[i]], [y[i - 1], y[i]], arrow_size,
                          **kw))
                i += interval
        else:
    ...
    def set_ax(ax):
        try:
            ax.prev_xlim
        #如果不存在prev_xlim属性，则进行初始化，避免重复初始化
            ...
            #更改ax原有的clear()方法以实现清空画布时同步清空arrowlines列表，防止清空画布后遗留的arrowlines会重复绘制
            ax.clear = MethodType(new_clear, ax)
            ax._request_autoscale_view1 = ax._request_autoscale_view

            def new_request_autoscale_view(self,
                                           tight=None,
                                           scalex=True,
                                           scaley=True):
            ...
            #更改原有的_request_autoscale_view方法以使得每次坐标区的视图状态改变时箭头方向都能及时更新
